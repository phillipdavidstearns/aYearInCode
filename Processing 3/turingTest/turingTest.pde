...
int scl = 8, dirs = 15, rdrop = 5, lim = 128;
int res = 1, palette = 0, pattern = 2, soft = 0;
int dx, dy, w, h, s;
boolean border, invert, imgLoaded;
float[] pat;
PImage img;
int frame = 0;
...
void draw() {
  ...
  img.save("output/test/"+nfs(frame,4)+".png");
  frame++;
  } else{
    println("press 'l' to select an image");
  }
}
 
void keyPressed() {
  switch(key) {
    case 'l': open_file(); break;
    //case 'r': reset(); break;
...
void pattern() {
  ...
  for(int l=0; l<s; l++) { 
    ...
    switch(pattern) {
      ...
      case 3: for(int i=imin; i<=imax; i++) pnew[l] += prange[l][i] + pvar[l][i]/3; break;
    }
