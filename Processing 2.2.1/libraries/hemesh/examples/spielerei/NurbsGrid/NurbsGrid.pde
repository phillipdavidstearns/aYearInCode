...
smooth(8);
  ...
  WB_RBSplineSurface bsps=WB_NurbsSwungSurface.getSwungSurface(P, T, 1);
  surface=new HE_Mesh(new HEC_FromSurface().setSurface(bsps).setU(20).setV(10));

  surface=new HE_Mesh(new HEC_FromFrame().setFrame(surface).setStrutRadius(6).setAngleOffset(0.5).setStrutFacets(6));
  surface.subdivide(new HES_CatmullClark(),2);
  aabb=new HE_Mesh(new HEC_Box().setFromAABB(surface.getAABB(), 0));
