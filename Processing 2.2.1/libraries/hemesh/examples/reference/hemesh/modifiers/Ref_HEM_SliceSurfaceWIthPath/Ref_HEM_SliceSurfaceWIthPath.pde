...
void draw() {
  ...
  fill(255, 0, 0);
  noStroke();
  for (HE_Path path : modifier.getPaths ()) {
    for (HE_Face f : path.getPathInnerFaces ()) {
      render.drawFace(f);
    }
  }
  fill(0,255, 0);
  noStroke();
  for (HE_Path path : modifier.getPaths ()) {
    for (HE_Face f : path.getPathOuterFaces ()) {
      render.drawFace(f);
    }
  }
  noFill();
...
void createMesh() {
  ...
  creator.setFacets(8).setSteps(4).setRadius(50).setHeight(400).setCap(false, true).setCenter(0, 0, 0);
