...
float make_bond = 50;
float break_bond = make_bond * 1.50;
float spring_length = make_bond * .75;
float spring_constant = 5.0;
float dampening = .99;
...
void oscillateNodes(){
  ...
  PVector _force = new PVector(50*sin(2 * TWO_PI * (counter/rate)), 50*sin( 3 * TWO_PI * (counter/rate)));
...
void mousePressed(){
  float _X = mouseX;
  float _Y = mouseY;
  PVector mousePos = new PVector(_X,_Y);
  if(mouseButton == LEFT){
    nodes.add(new Node(/*args*/));
  }
  if(mouseButton == RIGHT){
    for(int i = nodes.size()-1 ; i >=0 ; i--){
      Nodes node = nodes.get(i);
      if(PVector.dist(mousePos, node.pos)*2 <= node.r){
        println("here");
        node.remove(i);
        break;
      }
